    ...
    if (ext === '.json') {
        ...
        if (_.isObject(json)) {
            ...
            Object.keys(json).forEach(function (key) {
                ...
                // var cleanReplacement =  path.basename(json[key]).replace(new RegExp( opts.revSuffix ), '' ); --sumi
                if (!~[
                        path.basename(key),
        ...
        for (var key in manifest) {
            ...
            if ( dirReplacements.length ) {
                dirReplacements.forEach(function (dirRule) {
                    patterns.forEach(function (pattern) {
                        changes.push({
                            ...
                            // regexp: new RegExp(  dirRule.dirRX + pattern, 'g' ),  --sumi
                ...
                patterns.forEach(function (pattern) {
                    ...
                    prefixDelim += '])';
                    changes.push({
                        regexp: new RegExp( prefixDelim + pattern+'(\\?v=\\w{8,10})?', 'g' ),
                        //new RegExp( prefixDelim + pattern, 'g' ),   --sumi
